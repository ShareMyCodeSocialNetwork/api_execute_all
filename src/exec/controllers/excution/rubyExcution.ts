 ...
 */
export async function rubyExcution(request: Request, response: Response)
{
    const code = request.body.code;

    exec("docker start execrb-container", (error, stdout, stderr) => {
        if (error) {
            console.log(`error: ${error.message}`);
            return;
        }
        console.log(`start execrb-container`);
    });
